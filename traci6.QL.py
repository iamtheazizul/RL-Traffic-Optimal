...
for step in range(TOTAL_STEPS):
    ...
    state = get_state()                      # return current state as a tuple
    action = get_action_from_policy(state)   # use greedy-epsilon to get action
    apply_action(action)                     # apply action 0 (no c) or 1 (change)
